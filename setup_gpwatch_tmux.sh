...
tmux has-session -t astronomy 2>/dev/null

if [ $? != 0 ]; then
  echo "Creating a new tmux session 'astronomy'..."
  tmux new-session -d -s astronomy -n setup # Create a first window (temporary) for setup
else
  echo "tmux session 'astronomy' already exists. Attaching..."
  tmux attach-session -t astronomy
  exit 0
fi
...
for obs in "${observatories[@]}"; do
  ...
  tmux new-window -t astronomy -n "$obs"
...
tmux select-window -t astronomy:setup
